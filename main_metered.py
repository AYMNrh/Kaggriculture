...
# Town shop -> products each shop demands (decoded from env + replays).
# Single-product shops (YARN_STORE, PET_CAFE) eat 2x per tick.
SHOP_GOODS = {
    "BAKERY": ["EGG", "WHEAT"],
...
# Inverse: product -> shop types that demand it (town-demand robustness).
# WOOL has 1 shop (34.4% of 1.32.6 games it never opens = wool worthless),
# MILK 3 (2.3% miss), STRAWBERRY 4 (0.4% miss), WHEAT 5 (never missing).
SHOP_GOODS_INV = {}
for _s, _goods in SHOP_GOODS.items():
    for _g in _goods:
        SHOP_GOODS_INV.setdefault(_g, []).append(_s)
...
def agent(obs):
    ...
    #
    # ---- METERED SELLING (live-meta strategy, 1.32.6) ----
    # The price curves are CLIFFS: from equilibrium, STRAWBERRY hits the
    # $1 floor after only 62 units of oversupply, WOOL 59, MILK 76, MELON
    # 158. Selling a big order self-crashes the price WHILE you sell (the
    # market quotes each unit at the incremented inventory). The top farms
    # (live replay tracker, 675 episodes) sell in small metered batches:
    #   strawberry avg 12.7/order, melon 17.4, milk 7.2, wool 9.6,
    #   wheat 8.1, fertilizer 6.4
    # and drip through the day so town consumption (every 4 turns) raises
    # the price back between orders. Metering is the single biggest
    # strategy lesson of the live meta.
    ...
    # Town-adaptive batches: products whose shops are MISSING from this
    # game's draw have no town demand — their price only falls, so sell
    # them FAST (bigger batch, sooner). Products with open shops have
    # supported/rising prices — meter them tightly. Meta sell rhythm as
    # the base; scale up when demand is absent.
    _unlocked = set((obs.get("town") or {}).get("unlocked_shops", []))
    _no_demand = {item for item in SHOP_GOODS_INV
                  if not any(s in _unlocked for s in SHOP_GOODS_INV[item])}
    SELL_BATCH_BASE = {
        "STRAWBERRY": 12, "MELON": 17, "MILK": 7, "WOOL": 9,
        "WHEAT": 8, "FERTILIZER": 6, "CARROT": 12, "TOMATO": 12, "EGG": 12,
    }
    for item, qty in shed.items():
        ...
        if item == "WHEAT":
            ...
            if surplus > 0:
                # wheat has NO cliff (3000 units to floor) — sell freely
                market_orders.append(["SELL", "WHEAT", min(surplus, 30)])
        elif item in ("GOOSE", "COW", "SHEEP"):
            continue  # never sell animals
        else:
            batch = SELL_BATCH_BASE.get(item, 10)
            if item in _no_demand and item not in ("FERTILIZER",):
                # no shop wants it this game -> price only decays -> dump
                # faster (2x batch) but still not a cliff-crashing flood
                batch = min(2 * batch, 30)
            market_orders.append(["SELL", item, min(qty, batch)])
    ...
    for item, qty in shed.items():
        ...
        if item == "WHEAT":
            ...
            qty = max(0, surplus)
        # NOTE (yhay81): keep the budget mirror EXACTLY as baseline (full
        # qty) — changing it alters seed/animal buys, which changes tile
        # occupancy, which RE-ROLLS the town on every seed (the seed does
        # not fix the town). Metering must be a pure market-order change
        # to be A/B-testable. Phantom income is bounded by the 0.6 factor.
