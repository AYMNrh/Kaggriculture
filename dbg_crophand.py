"""Trace crop hands day 13 — label by role."""
import main
from kaggle_environments import make
orig = main.agent
log = open("crophand.txt", "w")

def wrapped(obs):
    res = orig(obs)
    me = obs["farms"][obs["player"]]
    day, hour = obs.get("day"), obs.get("hour")
    if day == 13 and 4 <= hour <= 12:
        ...
        for name, a in acts:
            tag = "C" if (name == "F" and 0 >= animal_count) or (name.startswith("h") and int(name[1:]) + 1 >= animal_count) else "A"
